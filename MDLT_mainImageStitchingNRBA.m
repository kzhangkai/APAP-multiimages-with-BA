...
mex ceresRigidError.cpp ...
 '-LD:/ceres/install/glog/lib' ...
 '-LD:/ceres/install/gflags/lib' ...
 '-LD:/ceres/install/ceres/lib' ...
'-ID:/ceres/install/ceres/include' ...
'-ID:/ceres/install/glog/include' ...
'-ID:/ceres/install/gflags/include' ...
'-ID:/ceres/eigen3' ...
-lceres ...
-lglog ...
-lgflags ;

mex ceresNonrigidError.cpp ...
 '-LD:/ceres/install/glog/lib' ...
 '-LD:/ceres/install/gflags/lib' ...
 '-LD:/ceres/install/ceres/lib' ...
'-ID:/ceres/install/ceres/include' ...  %-Include
'-ID:/ceres/install/glog/include' ...
'-ID:/ceres/install/gflags/include' ...
'-ID:/ceres/eigen3' ...
-lceres ...
...
% 此处 ceres.dll glog.dll gflags.dll 均在系统环境变量汇中包含(或者直接放在当前运行目录-本机方案)
...
imgs_pairs = combnk(1:num_imgs,2); % <-- Generating image pairs with all available image idexes.  所有图像组成的对数
imgs_pairs = [imgs_pairs ones(size(imgs_pairs,1),1)]; % Last column indicates if the pair is valid or not.  增加成3列
                                
%-----------------------------------------------------
% Keypoint detection and matching between image pairs.  计算每对图像间的特征点匹配对
%-----------------------------------------------------
fprintf('Pairwise keypoint detection and matching\n');

% Memory allocation  
images  = cell(num_imgs,1);                     % 为每幅图像分配存储空间
inliers = cell(size(imgs_pairs,1),1);            % 为每组图像分配内点 存储空间
data_norm = cell(size(imgs_pairs,1),1);    % 计算每组图像特征点的归一化坐标和原始坐标
data_orig = cell(size(imgs_pairs,1),1);      
h  = cell(size(imgs_pairs,1),1);                  % 为每组匹配对 分配9的局部单应矩阵

% For each image pair we find positive image matches.  %剔除不可能匹配的图像，保存正样本图像
for i=1:size(imgs_pairs,1)
    % Read images.
    img1 = imresize(imread(sprintf('%s%s',folder_name,dir_folder(imgs_pairs(i,1)).name)),scale);  % scale表示图像缩放尺度
    img2 = imresize(imread(sprintf('%s%s',folder_name,dir_folder(imgs_pairs(i,2)).name)),scale);  % 

    % Pre-warp images
    if exist('projection','var') && strcmp(projection,'cylinder') == 1  % exist('projection','var') 如果变量projection 存在，且采用柱面投影
        fprintf('  (Pre-)Projecting images into a cylinder...');tic;
        [img1,encl_rect] = imageProjection(double(img1),size(img1,2),'cylinder');  % encl_rect 返回投影图像的最小闭包矩形坐标
        img1 = reshape(uint8(img1),size(img1,1),size(img1,2)/3,3);
        img1 = imcrop(img1,encl_rect);                                                                   % 只取最小闭包矩形内的图像作为img1
    ...
    % Save images.
    images{imgs_pairs(i,1)} = img1;    % 存在预先分配的cell中
    ...
    % Refine homography using DLT on inliers.   % 全局单应变化矩阵
    ...
    % Verify image match.                                   %筛选无效匹配图像的标准
...
valid_pairs = find(imgs_pairs(:,3));    % 筛选满足条件的图像匹配对
fprintf('done (%fs)\n',toc);

%---------------------------------------------
% Extract reference image and stitching order.   %接下来选择所有图像的参考图像，并规定拼接顺序
%---------------------------------------------
...
imgs_conn_graph = zeros(num_imgs,num_imgs); % 图像邻接图  0,1方阵表示
...
[~,ref_img] = max(sum(imgs_conn_graph,2));   % 选择公共特征点的图 作为参考图像

% Find stitching order.
stitching_order = {ref_img};    % 拼接顺序

% Already considered/ordered images.
ordered_imgs = ref_img; % The first ordered image is the reference image.
% Unconsidered images.
unordered_imgs = setdiff(1:num_imgs, ordered_imgs);  %  setdiff筛选出剩余未排序的图像
...
while ~isempty(unordered_imgs)    
    ...
    if edge_weight == 0 % There was no next image, we probably have a disconnected panorama.
        fprintf('\n  ');
...
for i=1:size(stitching_order,1)                             %  按照给定拼接顺序 stitching_order
    for j=1:size(stitching_order{i},2)-1                  
        for k=1:size(valid_pairs,1)
            if stitching_order{i}(j) == imgs_pairs(valid_pairs(k),2) && stitching_order{i}(j+1) == imgs_pairs(valid_pairs(k),1)                
                ...
                imgs_pairs(valid_pairs(k),2) = aux;      % 保持目标图像在左边

                aux = data_orig{valid_pairs(k)}(1:3,:);  % data_orig为齐次坐标
                ...
                h{valid_pairs(k)} = reshape(reshape(h{valid_pairs(k)},3,3)\eye(3),9,1);     % 求逆
...
% sort the paramaters following the image order.  %按照拼接顺序 将全局单应性矩阵排序
lm_params0  = [];
for i=1:num_imgs
    for j=1:size(stitching_order,1)
        if i == stitching_order{j}(end)
            lm_params0 = [lm_params0; H{j}(:)];  % 用参考图像到目标图像的rigid_H作为BA初始值
            break;
        end
    end
end
...
for i=1:num_imgs
    for j=1:size(all_kpts_per_img{i},2)
        ...
        for k=1:size(stitch_pairs,1)
            if imgs_pairs(stitch_pairs(k),1) == i        % 左边为参考图像
            ...
            elseif imgs_pairs(stitch_pairs(k),2) == i  % 右边为参考图像
...
%----------------------------------------------------
%  Image stitching using Non-Rigid Bundle Adjustment.  %使用MDLT 进行BA优化
%----------------------------------------------------
